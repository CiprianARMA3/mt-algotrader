...
class Config:
    ...
    # Market Regime & ADX Parameters (Enhanced)
    USE_MARKET_REGIME = True
    ADX_TREND_THRESHOLD = 25       # Minimum ADX to consider a market "Trending"
    ADX_STRONG_TREND_THRESHOLD = 40 # ADX Level indicating a very strong trend
    ADX_SLOPE_THRESHOLD = 0.5      # Minimum change in ADX to define "strengthening"
...
class AdvancedFeatureEngine:
    ...
    def calculate_features(self, df):
        ...
        # Integrated full ADX system with DI+ and DI-
        if Config.USE_MARKET_REGIME:
            ...
            # 2. Trend Direction (1 = Bullish, -1 = Bearish, 0 = Neutral)
            df['trend_direction'] = np.where(df['plus_di'] > df['minus_di'], 1, -1)
            
            # 3. ADX Slope (Trend Strengthening/Weakening)
            df['adx_slope'] = df['adx'].diff()
            
            # 4. DI Spread (Distance between buyers and sellers)
            df['di_spread'] = abs(df['plus_di'] - df['minus_di'])
            
            # 5. Enhanced Regime Classification
            # 0: Ranging, 1: Trending, 2: Super Trending
            conditions = [
    ...
    def calculate_adx(self, df, period=Config.ADX_PERIOD):
        ...
        # Smoothed values (Wilder's Smoothing approximated by Rolling Mean for simplicity 
        # or ewm can be used. Using Rolling Sum per standard simple definition for ML features)
...
class AdvancedEnsemble:
    ...
    def predict(self, df):
        ...
        if Config.USE_MARKET_REGIME and 'regime' in f_dict:
            ...
            # If ADX says Strong Trend, ensure ML signal matches DI direction
            if adx > Config.ADX_TREND_THRESHOLD:
                # If Trend is UP (Plus_DI > Minus_DI) but Model says SELL (0)
                if plus_di > minus_di and final_p == 0:
                    Logger.log(f"Signal filtered: Fighting Uptrend (ADX {adx:.1f})", "WARNING")
                    return None, 0.0, None, sub_preds
                
                # If Trend is DOWN (Minus_DI > Plus_DI) but Model says BUY (1)
                if minus_di > plus_di and final_p == 1:
                    Logger.log(f"Signal filtered: Fighting Downtrend (ADX {adx:.1f})", "WARNING")
                    return None, 0.0, None, sub_preds

            # 2. Logic: Adjust Confidence based on ADX Slope
            # If ADX is rising, the trend is strengthening -> Require higher confidence
            ...
            # 3. Logic: Super Trend (Regime 2)
            if regime == 2:  # ADX > 40
                confidence_threshold = Config.MIN_CONFIDENCE * 1.3
            
            if final_c < confidence_threshold:
                 Logger.log(f"Signal filtered: Low confidence ({final_c:.2f}) for ADX regime", "WARNING")
                 return None, 0.0, None, sub_preds
...
class EnhancedPriceActionAnalyzer:
    ...
    def calculate_pivot_points(df):
        ...
        last_bar = df.iloc[-2]  # Previous bar
        high = last_bar['high']
        low = last_bar['low']
    ...
    def calculate_optimal_entry_sl_tp(df, signal, current_price, atr, risk_reward_ratio=Config.MIN_RR_RATIO):
        ...
        pivots = EnhancedPriceActionAnalyzer.calculate_pivot_points(df)
