...
class LoggerConfig:
    ...
    INDICATOR_BUFFER_DAYS = 7   # Days to fetch BEFORE the target date for accurate indicators
    HISTORY_DAYS = 365          # <--- CHANGED: How many days of history to fetch on startup
...
class DataLogger:
    ...
    def get_sync_dates(self, lookback_days=None):
        """
        Calculates start dates based on lookback or defaults to current month.
        """
        now = datetime.now()
        
        if lookback_days:
            # Custom lookback (e.g., 365 days)
            # We set the start date to N days ago, at 00:00:00
            start_date = now - timedelta(days=lookback_days)
            start_date = start_date.replace(hour=0, minute=0, second=0, microsecond=0)
        else:
            # Default: Start of current month (lighter for real-time loop)
            start_date = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
        
        # Buffer is always applied to whichever start date we chose
        # This ensures moving averages are correct for the first day we save
        buffer_start = start_date - timedelta(days=LoggerConfig.INDICATOR_BUFFER_DAYS)
        
        return start_date, buffer_start
    ...
    def perform_sync(self, lookback_days=None):
        """
        Fetches data. If lookback_days is set, fetches that history.
        Otherwise defaults to current month.
        """
        # Pass the argument to get_sync_dates
        start_date, fetch_start = self.get_sync_dates(lookback_days)
        ...
        # Filter: Only save data belonging to the requested period
        # (The buffer data was only used for calculation)
        df_clean = df_processed[df_processed['time'] >= start_date].copy()

        # Save
        saved_paths = self.save_to_database(df_clean)
        return saved_paths
    ...
    def run_realtime(self):
        ...
        # --- STEP 1: HISTORICAL BACKFILL (Run Once) ---
        Logger.log(f"Performing historical backfill ({LoggerConfig.HISTORY_DAYS} days)...", "DATA")
        Logger.log("This may take a moment...", "INFO")
        try:
            # Fetch 365 days
            backfill_paths = self.perform_sync(lookback_days=LoggerConfig.HISTORY_DAYS)
            if backfill_paths:
                Logger.log(f"Backfill Complete: {len(backfill_paths)} daily files created/updated.", "SUCCESS")
            else:
                Logger.log("Backfill yielded no data.", "WARNING")
        except Exception as e:
            Logger.log(f"Backfill Failed: {e}", "ERROR")

        # --- STEP 2: REAL-TIME LOOP ---
        Logger.log("Entering real-time monitoring mode (Current Month Only)...", "INFO")
        ...
        try:
            while True:
                ...
                # 2. Market is OPEN
                # We use default arguments (None) to fetch only the current month.
                # This keeps the loop fast.
                saved_paths = self.perform_sync(lookback_days=None)
